...
sampler_ESrepl <- function (data,burnin=10000,iterations=20000,prior,type,Htype=1,assessment_values="auto",d_diff=0){
  ...
  #### Create matrix for q and p
  iter_qp <- matrix(NA,nrow=Htype*iterations,ncol=4)
  colnames(iter_qp) <- paste(c("q_cand","q_it","p_cand","p_it"))
...
#     
#     ###### SAVE q and p
#     iter_qp[i,] <- c(density_proposal_cand,density_proposal_prev,density_posterior_cand,density_posterior_prev)
#     
...
#     
  ...
  ## LN
    ln_density_proposal_cand <- dgamma(1/var_it_cand,shape=proposal_parameters[1],rate=proposal_parameters[2],log=T)
    ln_density_proposal_prev <- dgamma(1/iter_val[i,3],shape=proposal_parameters[1],rate=proposal_parameters[2],log=T)
    
    ln_density_posterior_cand <- (((-1)*nrow(data)/2)-prior[3,1]-1)*log(var_it_cand)-((1/(var_it_cand))*(prior[3,2]+(1/2)*sum((data[,1]-iter_val[i+1,1]+data[,3]*iter_val[i+1,2]*sqrt(var_it_cand))^2)))
    ln_density_posterior_prev <- (((-1)*nrow(data)/2)-prior[3,1]-1)*log(iter_val[i,3])-((1/(iter_val[i,3]))*(prior[3,2]+(1/2)*sum((data[,1]-iter_val[i+1,1]+data[,3]*iter_val[i+1,2]*sqrt(iter_val[i,3]))^2)))
    
    ###### SAVE q and p
    iter_qp[i,] <- c(ln_density_proposal_cand,ln_density_proposal_prev,ln_density_posterior_cand,ln_density_posterior_prev)
    
    ln_r_it <- (ln_density_proposal_prev-ln_density_proposal_cand+ln_density_posterior_cand-ln_density_posterior_prev)
    r_it <- exp(ln_r_it)
    if(is.na(r_it)==T) {r_it<-0}
  ...
  ## Output
  results <- list("Gibbs"=iter_val,"MH"=iter_MH[,(1:2)],"posterior_summary"=posterior_summary,"prior"=prior, "qp"=iter_qp)
...
#     }
    ...
    ### M runs ### LN
    for(m in (burnin+1):(Htype*iterations)){
      ln_density_proposal_star <- dgamma(1/assessment_values[3],shape=proposal_parameters[1],rate=proposal_parameters[2],log=T)
      ln_density_proposal_it <- dgamma(1/iter_MH[m,1],shape=proposal_parameters[1],rate=proposal_parameters[2],log=T)
      
      ln_density_posterior_star <- (((-1)*nrow(data)/2)-prior[3,1]-1)*log(assessment_values[3])-((1/(assessment_values[3]))*(prior[3,2]+(1/2)*sum((data[,1]-iter_val[m+1,1]+data[,3]*iter_val[m+1,2]*sqrt(assessment_values[3]))^2)))
      ln_density_posterior_it <- (((-1)*nrow(data)/2)*log(iter_MH[m,1])-prior[3,1]-1)-((1/(iter_MH[m,1]))*(prior[3,2]+(1/2)*sum((data[,1]-iter_val[m+1,1]+data[,3]*iter_val[m+1,2]*sqrt(iter_MH[m,1]))^2)))
      
      ln_r_m <- (ln_density_proposal_it-ln_density_proposal_star+ln_density_posterior_star-ln_density_posterior_it)
      r_m <- exp(ln_r_m)

      a_m <- min(1,r_m)
      iter_MH[m,3] <- a_m
      
      aq_m <- a_m*exp(ln_density_proposal_star)
      iter_MH[m,4] <- aq_m
    }
...
#     }
    ...
    ### J runs ###  LN
    for(j in (burnin+1):(Htype*iterations)){
      ln_density_proposal_it <- dgamma(1/iter_MH[j,1],shape=proposal_parameters[1],rate=proposal_parameters[2],log=T)
      ln_density_proposal_star <- dgamma(1/assessment_values[3],shape=proposal_parameters[1],rate=proposal_parameters[2],log=T)
      
      ln_density_posterior_it <- (((-1)*nrow(data)/2)-prior[3,1]-1)*log(iter_MH[j,1])-((1/(iter_MH[j,1]))*(prior[3,2]+(1/2)*sum((data[,1]-iter_val[j+1,1]+data[,3]*iter_val[j+1,2]*sqrt(iter_MH[j,1]))^2)))
      ln_density_posterior_star <- (((-1)*nrow(data)/2)-prior[3,1]-1)*log(assessment_values[3])-((1/(assessment_values[3]))*(prior[3,2]+(1/2)*sum((data[,1]-iter_val[j+1,1]+data[,3]*iter_val[j+1,2]*sqrt(assessment_values[3]))^2)))
      
      ln_r_t <- (ln_density_proposal_star-ln_density_proposal_it+ln_density_posterior_it-ln_density_posterior_star)
      r_t <- exp(ln_r_t)
    ...
    results_repl <-   list("Gibbs"=iter_val,"MH"=iter_MH,"posterior_summary"=posterior_summary,"Chib"=Chib_ln,"marginal"=Chib_ln_marginal_result,"phi"=phi,"prior"=prior,"iterations"=iterations,"burnin"=burnin, "qp"=iter_qp)
